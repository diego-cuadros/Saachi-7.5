  ...
  theme.Variants = (function() {
    ...
    Variants.prototype = Object.assign({}, Variants.prototype, {
      ...
      _onSelectChange: function({srcElement}) {
        ...
        this._updateImagesPersonalizedGallery(variant);
        this._updatePrice(variant);
        this._updatePriceOnAtc(variant);
      ...
      //Organizing images on screen according to selected color swatch
      _updateImagesPersonalizedGallery: function (variant) {
        console.log("updating variants");
        var visibleImageCountOnLoad = 1;
        var numImageIsOdd = false;
        var galleryProductInfoElements = document.querySelectorAll(
          ".gallery-product-info"
        );
        var images = document.querySelectorAll(
          ".grid__item.small--hide[media-alt]"
        ); // Select only grid__item elements with class small--hide
        var imagesOnMobile = document.querySelectorAll(
          ".grid__item.medium-up--hide[media-alt]"
        ); //Select only grid__item elements with class medium-up--hide

        const currentImgAlt = variant.featured_image
          ? variant.featured_image.alt
          : null;

        imagesOnMobile.forEach(function (img) {
          if (img.getAttribute("media-alt") === currentImgAlt) {
            // Place images with specific alt attribute at the top
            img.style.order = -1;
          } else {
            // Push images without the specific alt attribute down
            img.style.order = 1;
          }
        });
        ...
        if (visibleImageCountOnLoad > 0 && visibleImageCountOnLoad <= 3) {
          galleryProductInfoElements.forEach(function (element) {
            element.style.gridRow = `1 / span ${visibleImageCountOnLoad}`;
            console.log(visibleImageCountOnLoad);
            console.log("condition 1 met");
          });
        } else if (
          visibleImageCountOnLoad == 4 ||
          visibleImageCountOnLoad == 6
        ) {
          galleryProductInfoElements.forEach(function (element) {
            element.style.gridRow = "1 / span 4";
            console.log("condition 2 met");
          });
        } else if (
          visibleImageCountOnLoad >= 5 &&
          visibleImageCountOnLoad < 7
        ) {
          galleryProductInfoElements.forEach(function (element) {
            element.style.gridRow = "1 / span 3";
            console.log("condition 3 met");
          });
        } else if (visibleImageCountOnLoad >= 7 && numImageIsOdd == true) {
          galleryProductInfoElements.forEach(function (element) {
            element.style.gridRow = "1 / span 4";
            console.log("condition 4 met");
          });
        } else if (visibleImageCountOnLoad >= 7 && !numImageIsOdd) {
          galleryProductInfoElements.forEach(function (element) {
            element.style.gridRow = "1 / span 3";
        ...
        console.log("visible images", visibleImageCountOnLoad);
       ...
       //Update price on ATC Button by Diego Cuadros
      _updatePriceOnAtc: function (variant) {
       // Get the price element within the "Add to Cart" button
        var priceOnAtcTags = document.querySelectorAll('.price-on-atc');
                      
        // Create a formatter object with the user's locale
        ...
        var priceInDollars = variant.price / 100; // Assuming e.price is in cents
                      
         // Iterate over each price element
          priceOnAtcTags.forEach(function(tag) {
          var formattedPrice = formatter.format(priceInDollars);
          var displayPrice = formattedPrice + ' USD';
          tag.innerText = displayPrice;
          });
      },
...
// ::::::::::::::::::::::::::::::::: Match My Outfit 2.0 Scripts :::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
 $(document).ready(function(){
            $('.slick-slider-trigger').slick({
                infinite: true,
                slidesToShow: 2,
                slidesToScroll:1,
                prevArrow:'.mmof-arrow_prev',
                nextArrow:'.mmof-arrow_next',
                arrows:true,
                responsive:[
                  {
                    breakpoint: 768,
        ...
        $('.slick-container_modal').slick({
            infinite: true,
            slidesToScroll:1,
            prevArrow:'.arrow_prev_modal',
            nextArrow:'.arrow_next_modal',
            arrows:true
        });

            // Open modal
            $('.mmof-modal-trigger').on('click', function() {
              //$('.slick-container_modal').slick('refresh');
                $('.slick-container_modal').slick('slickGoTo', 0);
                $('body').addClass('modal-open');
                $('#mmof-modalWindow').show();
                
            });

            // Close modal
            $('.mmof-modal-close-btn').on('click', function() {
                $('body').removeClass('modal-open');
                $('#mmof-modalWindow').hide();
            });
            ...
            $(window).on('click', function(event) {
                if ($(event.target).is('#mmof-modalWindow')) {
                    $('body').removeClass('modal-open');
                    $('#mmof-modalWindow').hide();
                }
            });
